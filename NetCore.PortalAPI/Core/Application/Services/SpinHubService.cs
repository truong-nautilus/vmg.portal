...
namespace ServerCore.PortalAPI.Core.Application.Services
{
	...
	public class SpinHubService: ISpinHubService
	{
		...
		private readonly ISpinHubRepository _spinHubRepository;
		public SpinHubService(IOptions<AppSettings> options, ISpinHubRepository spinHubRepository) {
			_appSettings =  options.Value;
			_spinHubRepository = spinHubRepository;
		}
		...
		{
			var userBalance = _spinHubRepository.GetUserBalance(userID);
			if (userBalance != null)
			{
		...
		{
			var playerbet = _spinHubRepository.PlayerBet(txId,token,currency,totalBet,timeStamp,ip,gameName,platform,gameId,betDetails,matchID);
		...
		{
			var playerWin = _spinHubRepository.PlayerWin(txId, token, currency, winCoin, timeStamp, ip, gameName, platform, gameId, isFreeTurn, matchID, totalBet);
		...
		{
			var playerCancelBet = _spinHubRepository.PlayerCancelBet(txId, token, currency, totalBet, timeStamp, gameName, platform, gameId, matchID, revTxId);

			return playerCancelBet;
		}
		public long PlayerLeave(int token, string gameName, string gameId, string time)
		{
			return _spinHubRepository.PlayerLeave(token, gameName, gameId, time);
		}
	}
}
