...
namespace PortalAPI.Services
{
    public class CoreAPI
    {
		...
		public CoreAPI(IOptions<AppSettings> options, IEventRepository eventRepository, IAccountRepository accountRepository, IReportRepository report, IGameTransactionRepository gameTransactionRepository, AccountSession accountSession, IAgencyRepository agencyRepository, ILoyaltyRepository loyaltyRepository)
        {
            this._appSettings = options.Value;
            this._eventRepository = eventRepository;
            this._report = report;
            this._gameTransactionRepository = gameTransactionRepository;
        ...
        public AccountDb Login(int platformId, int productId, string hashKey, string ipAddress, string userName, string authKey, string uiid)
        {
            try
            {
                ...
                int response;
                AccountDb account = _accountRepository.Login(userName, hashKey, ipAddress, uiid, productId, platformId, out response);
        ...
        public AccountDb LoginFacebook(string accessToken, string facebookId, string facebookEmail, int serviceId, int platformId, int productId, string ip, string uiid = "")
        {
            try
            {
                ...
                var accountChanneling = _accountRepository.SelectAccountByFacebook(3, fbInfor.Id, fbInfor.Email, listPartnerIDs, Convert.ToInt32(serviceId), Convert.ToInt32(platformId), productId, uiid);
        ...
        public long Transfer(int accountIdTrans, string nickNameTrans, int transferValue, long accountIdRecv, string nickNameRecv, int transferType, string ipAddress, int sourceId, string reason, int isWebAgency, string deviceid, out long balanceReceive, out long receiveAmount)
        {
            ...
            try
            {
                ...
                int status = _gameTransactionRepository.TransferBon(accountIdTrans, nickNameTrans, transferValue, accountIdRecv, nickNameRecv, transferType, ipAddress, sourceId, reason, isWebAgency, deviceid, out balanceUserTrans, out balanceReceive, out receiveAmount);
        ...
        public int GetTransferRate(string nickNameTrans, string nickNameRecv, out double rate)
        {
            ...
            try
            {
                rate = _agencyRepository.GetTransferRateNew(nickNameTrans, nickNameRecv);
        ...
        public bool UpdateSecurityInfo(long accountId, string email, string mobile, string passport, int avatarId)
        {
            try
            {
                int res = _accountRepository.UpdateInfo(accountId, email, mobile, passport, avatarId);
                if (res >= 0)
                    return true;
            }
            catch (Exception ex)
        ...
        public int UpdateMobile(long accountId, string mobile)
        {
            try
            {
                int res = _accountRepository.UpdateMobile(accountId, mobile);
        ...
        public int DeleteMobile(long accountId, string mobile)
        {
            try
            {
                int res = _accountRepository.DeleteMobile(accountId, mobile);
                NLogManager.Info(string.Format("accountId = {0}, mobile = {1}, res = {2}", accountId, mobile, res));
                return res;
            }
            catch (Exception ex)
        ...
        public int UpdateAvatar(long accountId, int avatar)
        {
            try
            {
                int res = _accountRepository.UpdateInfo(accountId, "", "", "", avatar);
                return res;
            }
            catch (Exception ex)
            {
        ...
        public int ChangePassWord(long accountId, string accountName, string oldPassWord, string newPassWord)
        {
            try
            {
                ...
                int resultChange = _accountRepository.ChangePassword(accountId, Security.TripleDESEncrypt(oldPassWord, oldPassWord), newPassEnc);
                ...
                if (resultChange >= 0)
                {
                    ...
                    //var writelog = _accountRepository.LogAccountChangepassWaiting_Insert(accountId, accountName, Security.MD5Encrypt(oldPassWord), Security.MD5Encrypt(newPassWord));
                    //if (writelog >= 0)
                    //    return resultChange;

                    // insert thông tin không thành công thì lại cập nhập lại pass cũ
                    //_accountRepository.ChangePassword(accountId, newPassEnc, account.Password);
                }
                return resultChange;
            }
            catch (Exception ex)
        ...
        {
            balance = -1;
            int res = _eventRepository.CheckGiftCode(accountId, accountName, nickName, giftCode, merchantId, merchantKey, sourceId, ipAddress, out balance);
        ...
        public AccountDb GetAccount(long accountId, string accountName)
        {
            ...
            return _accountRepository.GetAccount(accountId, accountName);
        }

        public int RegisterOTP(long accountId, string userName, string mobile, int register, out long balance)
        {
            return _accountRepository.RegisterOTP(accountId, userName, mobile, register, out balance);
        }
        ...
        {
            long res = _accountRepository.Frozen(accountId, bonFrozen, frozen ? _appSettings.ServiceIdFrozen : SERVICE_ID_UNFROZEN, frozen ? SERVICE_KEY_FROZEN : SERVICE_KEY_UNFROZEN, frozen, sourceId, clientIP, out balance, out frozenValue);
        ...
        {
            return _accountRepository.GetFrozenValue(accountId, out balance);
        }

        public List<TransferBonHistory> GetTransferBonHistory(int accountId)
        {
            if (accountId > 0)
                return _gameTransactionRepository.GetTransferBonHistory(accountId);
        ...
        {
            return _accountRepository.GetAccountByNickName(nickName, out userName, out accountId);
        }
        ...
        {
            return _accountRepository.GetChatStatus(accountID) == 1 ? true : false;
        }
        ...
        public dynamic GetVpLevel(int type)
        {
            try
            {
				return _loyaltyRepository.GetVpLevel(type);
			}
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        public dynamic FindVipPointInfo(long accountID)
        {
            try
            {
                return _loyaltyRepository.FindVipPointInfo(accountID);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        public dynamic FindTopAccountVpTransaction(long accountID, int top)
        {
            try
            {
                return _loyaltyRepository.FindTopAccountVpTransaction(accountID, top);
        ...
        public dynamic FindTopRankVipPointLevelInfo(long accountID, int top, int type)
        {
            try
            {
				...
				return _loyaltyRepository.FindTopRankVipPointLevelInfo(accountID, top, date ,type);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
