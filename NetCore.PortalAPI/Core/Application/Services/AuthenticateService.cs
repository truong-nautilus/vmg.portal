...
namespace ServerCore.PortalAPI.Core.Application.Services
{
    ...
    {
        // private readonly IEventRepository _eventRepository;//= AbstractRepositoryFactory.Instance().CreateEventRepository();
        private readonly IAccountRepository _accountRepository;// = AbstractRepositoryFactory.Instance().CreateAccountRepository();
        private readonly OTPSecurity _otp; //= OTPSecurity.Instance();
        private readonly IDataService _dataService;

        private readonly AppSettings appSettings;
        private readonly FacebookUtil _facebookUtil;
        private readonly IAgencyRepository _agencyDao;
        private readonly AccountSession _accountSession;
        private readonly IDistributedCache _cache;

        public AuthenticateService(AccountSession accountSession, IOptions<AppSettings> options, IEventRepository eventRepository, IAccountRepository accountRepository, IAgencyRepository acencyRepository, OTPSecurity otpSecurity, IDataService dataService, FacebookUtil facebookUtil, IDistributedCache cache)
        {
            appSettings = options.Value;
            // _eventRepository = eventRepository;
            _agencyDao = acencyRepository;
        ...
        public int Login(LoginAccount loginAccount, out AccountInfo accountInfo)
        {
            ...
            try
            {
                ...
                AccountDb account = _accountRepository.Login(loginAccount.UserName, loginAccount.Password, loginAccount.IpAddress, loginAccount.Uiid, loginAccount.MerchantId, loginAccount.PlatformId, out response);
                //AccountDb account = _accountRepository.Login(loginAccount.UserName, Security.GetPasswordEncryp(loginAccount.Password, loginAccount.UserName), loginAccount.IpAddress, loginAccount.Uiid, loginAccount.MerchantId, loginAccount.PlatformId, out response);
                //watch.Stop();
                ...
                //        }
                //        long accountID = -1;
                //        string userName = _accountRepository.GetInfoByMobile(loginAccount.UserName, out accountID);
                //        if (accountID > 0)
                //        {
                //            account = _accountRepository.Login(userName, Security.GetPasswordEncryp(loginAccount.Password, userName), loginAccount.IpAddress, loginAccount.Uiid, loginAccount.MerchantId, loginAccount.PlatformId, out response);
                //        }
                //    }
                ...
                SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(account.AccountID);
        ...
        public int GetInfo2(int merchantID, string partnerUserID, out AccountInfo accountInfo)
        {
            ...
            try
            {
                AccountDb account = _accountRepository.GetAccountInfo2(merchantID, partnerUserID);
        ...
        public int GetInfoByAccountID(int merchantID, long accountID, out AccountInfo accountInfo)
        {
            ...
            try
            {
                AccountDb account = _accountRepository.GetInfoByAccountID(merchantID, accountID);
                CacheCounter.AccountActionDelete(_accountSession.AccountName, appSettings.LoginActionName);
                if (account != null)
                {
                    SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(account.AccountID);
        ...
        public int GetInfoByUserName(int merchantID, string userName, out AccountInfo accountInfo)
        {
            ...
            try
            {
                AccountDb account = _accountRepository.GetInfoByUserName(merchantID, userName);
                CacheCounter.AccountActionDelete(_accountSession.AccountName, appSettings.LoginActionName);
                if (account != null)
                {
                    SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(account.AccountID);
        ...
        public Game GetGameUrlByGameID(byte gameID)
        {
            try
            {
                return _accountRepository.GetGameUrlByGameID(gameID);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        public int GetInfo(long accountID, string username, out AccountInfo accountInfo)
        {
            ...
            try
            {
                AccountDb account = _accountRepository.GetAccountInfo(accountID, username);
                CacheCounter.AccountActionDelete(_accountSession.AccountName, appSettings.LoginActionName);
                if (account != null)
                {
                    SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(account.AccountID);
        ...
        public async Task<AccountInfo> LoginFBAsync(LoginAccount fbData)
        {
            ...
            try
            {
                ...
                ChannelingAccount accountChanneling = _accountRepository.SelectAccountByFacebook(3, fbId, "", partnerIDS, merchantId, platformId, 0, passWord, uiid, fbData.IpAddress);
                ...
                if (accountChanneling == null || accountChanneling.ChannelingID <= 0)
                {
                    ...
                    //accountChanneling = _accountRepository.SelectAccountByFacebook(3, fbInfor.Id, fbInfor.Email, partnerAccountIds, Convert.ToInt32(serviceId), Convert.ToInt32(platformId), productID);
                }
                AccountDb account = _accountRepository.GetAccount(accountChanneling.AccountID, accountChanneling.AccountName);
                //elapsedMs = watch.ElapsedMilliseconds;
        ...
        //        ChannelingAccount accountChanneling = _accountRepository.SelectAccountByFacebook(3, fbInfor.Id, fbInfor.Name, listPartnerIDs, merchantId, platformId, 0, uiid);
        //        if (accountChanneling == null || accountChanneling.ChannelingID <= 0)
        //        {
        //            NLogManager.Info("accountChanneling_Error: " + fbId);
        //            return (int)ErrorCodes.LOGIN_ERROR;
        ...
        //        AccountDb account = _accountRepository.GetAccount(accountChanneling.AccountID, accountChanneling.AccountName);
        //        //int res = _accountRepository.UpdateUserFullName(accountChanneling.Mobile, accountChanneling.AccountID, fbName);
        //        if (account == null || account.AccountID < 1)
        ...
        public bool CheckUserNameIsMobileOther(string mobile)
        {
            ...
            long accountID = -1;
            string userName = _accountRepository.GetInfoByMobile(mobileFind, out accountID);
            if (accountID > 0)
                return true;
        ...
        public int Register(LoginAccount loginAccount, out AccountInfo accountInfo)
        {
            ...
            try
            {
                ...
                AccountDb accountDb = _accountRepository.Register(username, passwordEncryp, nickName, ipAddress, merchantId, platformId, campaignSource, uuid, out response, locationID, loginAccount.Email, loginAccount.PartnerUserID, loginAccount.WAddress);
                if (accountDb != null)
                {
        ...
        public async Task<AccountInfo> LoginAppleIdAsync(LoginAccount appleData)
        {
            ...
            try
            {
                ...
                if (securityToken != null)
                {
                    ...
                    ChannelingAccount accountChanneling = _accountRepository.SelectAccountByApple(appleId, bundleId, email, merchantId, platformId, uiid, appleData.IpAddress);
        ...
        public void UpdateTeleChatID(int accountID, string chatId)
        {
            try
            {
                _accountRepository.UpdateTeleChatID(accountID, chatId);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        public int GetInfoTele(string chatId, out AccountInfo accountInfo)
        {
            ...
            try
            {
                var accountTele = _accountRepository.GetSecurityInfoByTeleChatID(chatId).FirstOrDefault();
                if (accountTele != null)
                {

                    AccountDb account = _accountRepository.GetAccountInfo(accountTele.AccountID, "");
                    CacheCounter.AccountActionDelete(_accountSession.AccountName, appSettings.LoginActionName);
                    SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(account.AccountID);
                    if (securityInfo != null)
                        account.IsMobileActived = securityInfo.IsMobileActived;
        ...
        public int GetInfoByWAddress(string wAddress, out AccountInfo accountInfo)
        {
            ...
            try
            {
                var accountMeta = _accountRepository.GetSecurityInfoByWAddress(wAddress).FirstOrDefault();
                if (accountMeta != null)
                {

                    AccountDb account = _accountRepository.GetAccountInfo(accountMeta.AccountID, "");
                    CacheCounter.AccountActionDelete(_accountSession.AccountName, appSettings.LoginActionName);
                    SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(account.AccountID);
                    if (securityInfo != null)
                        account.IsMobileActived = securityInfo.IsMobileActived;
        ...
        {
            _accountRepository.CreateLoginSession(session);
        }
    }
}
