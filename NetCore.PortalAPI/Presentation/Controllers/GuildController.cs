...
namespace PortalAPI.Controllers
{
    ...
    public class GuildController : ControllerBase
    {
        ...
        public GuildController(AccountSession accountSession, IAgencyRepository agencyRepository, IGuildRepository guildRepository, IAuthenticateService authenticateService, IOptions<AppSettings> options)
        {
        ...
        public ActionResult<ResponseBuilder> GameAgencies()
        {
            try
            {
                ...
                List<GameAgency> listAg = _guildRepository.GetGameAgencies();
                ...
                if (AdminGuild != null)
                {
                   // AdminGuild.Members = _agencyRepository.GetListMember(_accountSession.AccountID, 1);
                    //AdminGuild.MemberRequests = _agencyRepository.GetListMember(_accountSession.AccountID, 0);
                  //  AdminGuild.MemberLefts = _agencyRepository.GetListMember(_accountSession.AccountID, 2);
                    foreach (GameAgency item in listAg)
                    {
                        if (item.AccountID != _accountSession.AccountID)
                        {
                            if (item.ContentRule == null)
                                item.ContentRule = "";
                        }
                        else
                        {
                ...
                {
                    long agencyID = _guildRepository.GetAgencyIDByMember(_accountSession.AccountID);
        ...
        public ActionResult<ResponseBuilder> UpdateRule(dynamic data)
        {
            ...
            try
            {
                ...
                int responseStatus = _guildRepository.UpdateRule(_accountSession.AccountID, content);
        ...
        public ActionResult<ResponseBuilder> GameAgencyDetail()
        {
            try
            {
                ...
                if (AdminGuild != null)
                {
                    AdminGuild.Members = _guildRepository.GetListMember(_accountSession.AccountID, 1);
                    AdminGuild.MemberRequests = _guildRepository.GetListMemberOther(_accountSession.AccountID, 0);
                   // AdminGuild.MemberLefts = _agencyRepository.GetListMemberOther(_accountSession.AccountID, 2);
                ...
                else // Member
                {
                    ...
                    if (agencyID > 0)
                    {
                        ...
                        if (MemGuild != null)
                        {
                            MemGuild.Members = _guildRepository.GetListMember(MemGuild.AccountID, 1);
        ...
        public ActionResult<ResponseBuilder> RequestJoinAgency(dynamic data)
        {
            ...
            int res = _guildRepository.RequestJoinAgency(agencyID, _accountSession.AccountID, _accountSession.AccountName, _accountSession.NickName);
        ...
        public ActionResult<ResponseBuilder> GetListMember(dynamic data)
        {
            try
            {
                ...
                List<GameMember> list = _guildRepository.GetListMember(agencyAccountId, status);
        ...
        public ActionResult<ResponseBuilder> AcceptRequest(dynamic data)
        {
            try
            {
                ...
                int isAgency = 0;
                // Check agency
                AgenciesInfo agency = _agencyRepository.AgencyGetInfo(_accountSession.NickName);
                if (agency.AgencyID < 0 || agency.LevelID > 1)
                {
                    ...
                    List<GameJoinRequest> rqJoined = _guildRepository.GetListRequestJoinAgency((int)_accountSession.AccountID, 1);
                    if (rqJoined == null || rqJoined.Count < 1) // Chưa join Bang nào
                    {
                        return new ResponseBuilder(ErrorCodes.FORBIDDEN_ACCESS, _accountSession.Language);
                    }
                    // Đã gửi yêu cầu join bang nào chưa?
                    List<GameJoinRequest> rq = _guildRepository.GetListRequestJoinAgency((int)_accountSession.AccountID, 10);
                ...
                int res = _guildRepository.AcceptRequestInfo(agencyId, nickName, status, isAgency);
                if (res == -96)
                {
