...
namespace PortalAPI.Controllers
{
    ...
    public class PrivacyController : ControllerBase
    {
        ...
        public PrivacyController(OTPSecurity otp, IAccountRepository accountRepository, CoreAPI coreAPI, AccountSession accountSession)
        {
        ...
        public ActionResult<ResponseBuilder> VerifyOTP(OtpModel model)
        {
            try
            {
                ...
                switch ((OtpServiceCode)model.ServiceId)
                {
                    ...
                    //        AccountDb account = _accountRepository.GetAccount(otpData.AccountId, loginData.UserName);
                    ...
                    case OtpServiceCode.OTP_SERVICE_CHANGE_PASSWORD:
                        {
                            ...
                            string newPassEnc = data.NewPassWord;// Security.GetPasswordEncryp(data.NewPassWord, data.AccountName);
                            string oldPassEnc = data.OldPassWord;// Security.GetPasswordEncryp(data.OldPassWord, data.AccountName);
                            int res = _accountRepository.ChangePassword(data.AccountID, oldPassEnc, newPassEnc);
                            string content = "";
                            if(res == -1)
