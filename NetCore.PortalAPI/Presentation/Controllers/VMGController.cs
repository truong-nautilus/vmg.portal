...
namespace ServerCore.PortalAPI.Presentation.Controllers
{
    ...
    {
        private IVMGRepository _vmgRepository;
        private IAccountRepository _accountRepository;
        private IAuthenticateService _authenticateService;
        private AccountSession _accountSession;
        private string lng;
        public VMGController(IVMGRepository vmgRepository, IAuthenticateService authenticateService, IAccountRepository accountRepository, AccountSession accountSession)
        {
            _vmgRepository = vmgRepository;
        ...
        public ActionResult<ResponseBuilder> GetChip()
        {
            ...
            try
            {
                var result = _vmgRepository.GetChipByCurrencyID(_accountSession.CurrencyID);
        ...
        {
            var agency = _vmgRepository.GetAgentById(AgentId);
        ...
        public ActionResult<ResponseBuilder> VMGApi(VMGRequest request)
        {
            try
            {
                ...
                // giải mã data
                var agency = _vmgRepository.GetAgentById(request.AgencyId);
                ...
                switch (request.Method)
                {
                    ...
                    case "GetBalance":
                        ...
                        if (response_gb == 0)
                        {
                            var lstBalanceAccount = _accountRepository.GetListBalanceAccount(account_gb.AccountID);
                    ...
                    case "DepositUser":
                        ...
                        if (response_du == 0)
                        {
                            var res_du = _vmgRepository.DepositUser(account_du.AccountID, account_du.UserName, agency.AgencyId, depositUserParam.Amount, depositUserParam.TransId, out balance_du);
                            if (res_du > 0)
                    ...
                    case "CashoutUser":
                        ...
                        if (response_ca == 0)
                        {
                            var res_ca = _vmgRepository.CashoutUser(account_ca.AccountID, account_ca.UserName, agency.AgencyId, cashoutParam.Amount, cashoutParam.TransId, out balance_ca);
                            if (res_ca > 0)
                    ...
                    case "BetHistory":
                        ...
                        //if (response_bh == 0)
                        //{
                        var betHistories = _vmgRepository.GetAccountBetHistory(betHistoryParam.FromDate, betHistoryParam.ToDate, agency.AgencyId, out totalPage, betHistoryParam.PageNumber, betHistoryParam.PageSize);
