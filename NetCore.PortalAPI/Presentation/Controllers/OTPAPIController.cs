...
// using static Microsoft.AspNetCore.Hosting.Internal.HostingApplication; // Not available in .NET 8
...
namespace PortalAPI.Controllers
{
    ...
    public class OTPAPIController : ControllerBase
    {
        ...
        public OTPAPIController(IOTPRepository iOTPRepository, IAccountRepository accountRepository, IAgencyRepository agencyRepository, AccountSession accountSession, OTPSecurity otpSecurity, IAuthenticateService authenticateService, Captcha captchaUtil)
        {
            this._iOTPRepository = iOTPRepository;
            this._accountRepository = accountRepository;
            this._agencyDao = agencyRepository;
        ...
        public ActionResult<ResponseBuilder> SetupAppToken(OTPs.SetupAppTokenInput input)
        {
            ...
            {
                SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(_iAccountID);
            ...
            try
            {
                ...
                if (_dataOutput.Status == 1)
                {
                    ...
                    else
                    {
                        ...
                        //int iResultSql = _iOTPRepository.AccountVerifyOTP(_accountVerifyOtpInput);
                        ...
                        if (code == OTPCode.OTP_VERIFY_SUCCESS)
                        {
                            ...
                            OTPs.ResponMes responMes = _iOTPRepository.SetupAppToken(_dataOutput);
                ...
                else if (_dataOutput.Status == 9)
                {
                    ...
                    int iResultSql = 1;//_iOTPRepository.AccountVerifyOTP(_accountVerifyOtpInput); huy DK OTP app thi ko can check OTP

                    if (iResultSql > 0)
                    {
                        //  Gửi lại bản tin lần 2
                        OTPs.ResponMes responMes2 = _iOTPRepository.SetupAppToken(_dataOutput);
        ...
        public ActionResult<ResponseBuilder> CheckTokenAvailable(OTPs.TokensCheckActiveInput input)
        {
            try
            {
                ...
                int iCheckActive = _iOTPRepository.TokensCheckActive(_checkActive);
        ...
        public ActionResult<ResponseBuilder> SyncTime(OTPs.CheckActiveSyncTimeInput input)
        {
            try
            {
                ...
                int iResultSync = _iOTPRepository.TokensSyncTime(_activeSyncTime);
        ...
        public ActionResult<ResponseBuilder> UpdateAccountInfo(OTPs.AccountUpdateInfoInput input)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(input.otp))
                {
                    ...
                    int _iDBUpdate = _iOTPRepository.UpdateInfo(_updateInfo);
                ...
                else
                {
                    ...
                    int _iSQLResult = _iOTPRepository.AccountVerifySMS(_accountVerifySms);
        ...
        public ActionResult<ResponseBuilder> ActiveTwoFactor(string uiid)
        {
            try
            {
                ...
                int res = _iOTPRepository.ActiveTwoFactor(activeTwoFactor);
                if (res < 0)
        ...
        public ActionResult<ResponseBuilder> TwoFactorVerify(string code)
        {
            try
            {
                ...
                string tokenKey = "";
                string secretKey = "";
                int res = _iOTPRepository.GetTwoFactorInfo(_accountSession.AccountID, out tokenKey, out secretKey);
                if (res < 0)
                {
        ...
        public ActionResult<ResponseBuilder> getOtpVerify(int AccountID, string AccountName, string mobile, int type)
        {
            ...
            {
                int codeVerify = 0;
                int res = _accountRepository.CreateSecuritySMS(AccountID, AccountName, mobile, type, ref codeVerify);
                if (res > 0)
                {
        ...
        public ActionResult<ResponseBuilder> getOtpSMS(dynamic model)
        {
            ...
            try
            {
                ...
                SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(_accountSession.AccountID);
        ...
        public ActionResult<ResponseBuilder> getOtpSMS1(dynamic model)
        {
            ...
            try
            {
               ...
               //     return new ResponseBuilder(ErrorCodes.ACCOUNT_VERIFY_CODE_INVALID, _accountSession.Language);
                ...
                SecurityInfo securityInfo = _accountRepository.GetSecurityInfo((int)otpData.AccountId);
        ...
        public ActionResult<ResponseBuilder> LoginVerifyOTP([FromBody] OtpModel model)
        {
            try
            {
                ...
                // Xác thực qua API
                int rs = _otpSecurity.VerifyOtpByNewApi(otpData.AccountName, otpData.AccountId, securityInfo.Mobile, model.Otp, type);

                if (rs != 0) return new ResponseBuilder(ErrorCodes.ACCOUNT_OTP_NOT_MATCH, _accountSession.Language);

                // GenerateToken trả về
                AccountDb account = _accountRepository.GetAccount(otpData.AccountId, otpData.AccountName);
                account.IsAgency = _agencyDao.CheckIsAgency((int)account.AccountID);
                account.IsMobileActived = securityInfo.IsMobileActived;
                account.PlatformID = otpData.PlatformId;
        ...
        public ActionResult<ResponseBuilder> ChangePassVerifyOTP([FromBody] OtpModel model)
        {
            try
            {
                ...
                string newPassEnc = data.NewPassWord;// Security.GetPasswordEncryp(data.NewPassWord, _accountSession.AccountName);
                string oldPassEnc = data.OldPassWord;// Security.GetPasswordEncryp(data.OldPassWord, _accountSession.AccountName);

                int response = _accountRepository.ChangePassword(_accountSession.AccountID, oldPassEnc, newPassEnc);
        ...
        public ActionResult<ResponseBuilder> VerifyOTP(dynamic model)
        {
            try
            {
                ...
                int rs = _otpSecurity.VerifyOtpByNewApi(_accountSession.AccountName, _accountSession.AccountID, securityInfo.Mobile, otp, type);
                if (rs != 0)
        ...
        public ActionResult<ResponseBuilder> TelegramOTP(dynamic model)
        {
            ...
            try
            {
                ...
                SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(_accountSession.AccountID);
                NLogManager.Info("securityInfo: " + JsonConvert.SerializeObject(securityInfo));
                ...
                NLogManager.Info("TelegramOTP: " + JsonConvert.SerializeObject(rs));
