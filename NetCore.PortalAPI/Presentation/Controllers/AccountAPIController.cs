...
namespace PortalAPI.Controllers
{
    ...
    public class AccountAPIController : ControllerBase
    {
        ...
        public AccountAPIController(AccountSession accountSession, IAuthenticateService authenticateService, OTPSecurity otp, IAccountRepository accountRepository, IAgencyRepository agencyRepository, IEventRepository eventRepository, IMobileRepository mobileRepository, CoreAPI coreAPI, IOTPRepository otpRepository, IOptions<AppSettings> options, Captcha captcha)
        {
        ...
        [HttpGet("GetLocation")]
        public ActionResult<ResponseBuilder> GetLocation()
        {
            try
            {
                var result = _accountRepository.GetLocationRegister();
                if (result.Count > 0)
                {
                    return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, result);
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("GetMobileCodes")]
        public ActionResult<ResponseBuilder> GetMobileCodes()
        {
            try
            {
                var result = _accountRepository.GetMobileCodes();
                if (result.Count > 0)
                {
                    return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, result);
                }
        ...
        public ActionResult<ResponseBuilder> UpdateAccount([FromBody] dynamic data)
        {
            try
            {
                ...
                var result = _accountRepository.UpdateInfo(accountId, "", mobile, passport, avatar);
        ...
        public ActionResult<ResponseBuilder> UpdateSecurityInfoReport([FromBody] dynamic data)
        {
            try
            {
                ...
                var result = _accountRepository.UpdateInfo(accountId, "", mobile, "", -1);
        ...
        public ActionResult<ResponseBuilder> ChangePassword(ChangePasswordData data)
        {
            try
            {
                ...
                if (accountName.StartsWith("FB.") && oldpass.CompareTo(newpass) == 0)
                {
                    AccountEbank accountCheckFB = _accountRepository.Get(_accountSession.AccountID);
                    string fbId = accountName.Replace("FB.", "");
                    string passWord = "6b4b2bf6d0d1cf8aecdcc74a01cdac31";// Security.GetPasswordEncryp("6b4b2bf6d0d1cf8aecdcc74a01cdac31", "FB."+fbId);
                    if (passWord.CompareTo(accountCheckFB.Password) != 0)
                        return new ResponseBuilder(ErrorCodes.PASSWORD_FB_INVALID, _accountSession.Language);
                    else
                        oldpass = "6b4b2bf6d0d1cf8aecdcc74a01cdac31";
                }
                ...
                AccountEbank account = _accountRepository.Get(_accountSession.AccountID);

                string newPassEnc = newpass;// Security.GetPasswordEncryp(newpass, accountName);
                string oldPassEnc = oldpass;//Security.GetPasswordEncryp(oldpass, accountName);
                ...
                else
                {
                    ...
                    int response = _accountRepository.ChangePassword(accountId, oldPassEnc, newPassEnc);
                    if (response >= 0)
                        return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language);
        ...
        public ActionResult<ResponseBuilder> UpdateEmail(string email)
        {
            try
            {
                ...
                //AccountDb accountDB = _accountRepository.GetAccount(acountID, userName);
                //if(accountDB != null)
                //{
                ...
                int res = _mobileRepository.UpdateEmail(_accountSession.AccountID, email);
        ...
        public ActionResult<ResponseBuilder> GiftCode(Models.Giftcode input)
        {
            ...
            try
            {
                ...
                if (res >= 0)
                {
                    //string message = String.Format("Bạn nạp thành công Code: {0}! Mệnh giá {1}.Vào lúc {2}. Chúc bạn chơi game vui vẻ", input.giftCode, res, DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss"));
                    _otp.PushMessageToTelegram(_accountSession.AccountID, _accountSession.AccountName, input.giftCode, res, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"));
                ...
                else
                {
                    ...
                    string content = "GiftCode không hợp lệ.";
                    if (res == -100)
                        content = "GiftCode đã được sử dụng.";
                    else if (res == -101)
                        content = "GiftCode không tồn tại.";
                    else if (res == -102)
                        content = "GiftCode không được dùng trong thời gian này.";
                    else if (res == -103)
                        content = "GiftCode đã hết hạn.";
                    else if (res == -104)
                        content = "GiftCode đã bị khóa.";
                    else if (res == -105)
                        content = "Bạn đã hết số lượt nhập Giftcode trong sự kiện này.";
                    else if (res == -106)
                        content = "GiftCode không được sử dụng cho hệ điều hành này.";
                    else
                        content = "Nạp GiftCode không thành công. Vui lòng liên hệ nhà phát hành.";
        ...
        public ActionResult<ResponseBuilder> GiftCode(string giftCode, string captcha, string verifyCaptcha)
        {
            ...
            try
            {
                ...
                if (res >= 0)
                {
                    //string message = String.Format("Bạn nạp thành công Code: {0}! .Vào lúc {1}. Chúc bạn chơi game vui vẻ", giftCode, DateTime.Now.ToString("MM/dd/yyyy HH:mm:ss"));
                    _otp.PushMessageToTelegram(_accountSession.AccountID, _accountSession.AccountName, giftCode, res, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"));
        ...
        private ActionResult<ResponseBuilder> CheckAccountExists(int accountId)
        {
            try
            {
                ...
                var checkAccountExist = _accountRepository.Get(accountId);
        ...
        public ActionResult<ResponseBuilder> ResetPassword(ResetPassModels model)
        {
            try
            {
                ...
                // var checkAccountID = _accountRepository.CheckAccountExist(model.AccountName);
                // if (checkAccountID <= 0)
                //    return new ResponseBuilder(ErrorCodes.ACCOUNT_NOT_EXIST, _accountSession.Language);
                // Lấy thông tin account cũ, trước khi Reset Pass
                AccountEbank account = _accountRepository.Get(otpData.AccountId);
                // if (model.Type == 1) // SMS
                //{
                ...
                // Kiểm tra thông tin bảo mật OTP
                var securityInfo = _accountRepository.GetSecurityInfo(otpData.AccountId);
                ...
                if (_otp.VerifyOtpByNewApi(otpData.AccountName, otpData.AccountId, securityInfo.Mobile, otp, otpType) != 0)
                    return new ResponseBuilder(ErrorCodes.ACCOUNT_OTP_NOT_MATCH, _accountSession.Language);

                string oldPass = account.Password;
                //int result_reset = _accountRepository.ResetPassword(account.AccountName, Security.GetPasswordEncryp(newPass, account.AccountName), (short)model.Type);
                int result_reset = _accountRepository.ResetPassword(account.AccountName, newPass, (short)model.Type);
                if (result_reset < 0)
                {
                    // Cập nhật lại Mật khẩu cũ khi reset failed
                    _accountRepository.ResetPassword(account.AccountName, oldPass, (short)model.Type);
                    return new ResponseBuilder(ErrorCodes.ACCOUNT_RESET_PASSWORD_FAILED, _accountSession.Language);
                }
                NLogManager.Info(string.Format("Account = {0} reset mat khau thanh cong", account.AccountName));
                return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<ResponseBuilder> ResetPasswordCheck(dynamic model)
        {
            try
            {
                ...
                var accountID = _accountRepository.CheckAccountExist(accountName);
                if (accountID <= 0)
                    return new ResponseBuilder(ErrorCodes.ACCOUNT_NOT_EXIST, _accountSession.Language);
        ...
        public ActionResult<ResponseBuilder> HardResetPassword(ResetPassModels model)
        {
            try
            {
                ...
                AccountEbank account = _accountRepository.Get(checkAccountID);
                ...
                if (model.Type == 9) // Hard reset
                {
                    ...
                    string oldPass = account.Password;
                    //int result_reset = _accountRepository.ResetPassword(account.AccountName, Security.GetPasswordEncryp(model.NewPassword, model.AccountName), (short)model.Type);
                    int result_reset = _accountRepository.ResetPassword(account.AccountName, model.NewPassword, (short)model.Type);
                    if (result_reset < 0)
                    {
        ...
        public ActionResult<ResponseBuilder> UpdateUserFullName(string fullName, string captcha, string captchaToken, string refCode = "")
        {
            try
            {
                ...
                var accountInfo = _accountRepository.GetAccount(_accountSession.AccountID, _accountSession.AccountName);
                if (accountInfo == null)
                {
                    return new ResponseBuilder(ErrorCodes.ACCOUNT_NOT_EXIST, _accountSession.Language);
                }
                ...
                int response = _accountRepository.UpdateUserFullName(_accountSession.AccountName, _accountSession.AccountID, fullName, refCode);
                ...
                if (response >= 0)
                {
                    ...
                    accountDb.IsAgency = _agencyRepository.CheckIsAgency((int)accountDb.AccountID);
                    accountDb.LocationID = _accountSession.LocationID;
                    accountDb.PreFix = _accountSession.PreFix;
        ...
        public ActionResult<ResponseBuilder> UpdateUserFullNameFB(string fullName, string userName, string password, string captcha, string captchaToken)
        {
            try
            {
                ...
                int response = _accountRepository.UpdateUserFullNameFB(_accountSession.AccountID, userName, passwordEncryp, fullName);
                if (response >= 0)
                {
                    AccountDb accountDb = new AccountDb();
                    accountDb.AccountID = _accountSession.AccountID;
        ...
        public ActionResult<ResponseBuilder> GetSecurityInfo()
        {
            try
            {
                ...
                //_accountRepository.GetSecurityInfo((int)accountSession.AccountID);
                if (securityInfo != null)
                {
            ...
            try
            {
                ...
                if (securityInfo != null)
                {
                    ...
                    if (model.Type == 1) // Đăng ký bảo mật
                    {
                        ...
                        if (_otp.VerifyOtpByNewApi(_accountSession.AccountName, _accountSession.AccountID, userMobile, model.SecureCode, model.OtpType) != 0)
                            return new ResponseBuilder(ErrorCodes.ACCOUNT_OTP_NOT_MATCH, _accountSession.Language);

                        //if (!_otp.
                        //
                        //
                        //(_accountSession.AccountName, (int)_accountSession.AccountID, model.SecureCode, 1, securityInfo.Mobile, 0))
                        //    return new ResponseBuilder(ErrorCodes.ACCOUNT_OTP_NOT_MATCH, _accountSession.Language);
                        ...
                        if (res >= 0)
                        {
                            if (balance > 0)
                            {
                                string notificationSend = string.Format("Ngày [{0}] tài khoản [{1}] Nhận số tiền là: {2:0,0}. Chúc mừng bạn đã đến với cổng game uy tín nhất VN!. Chúc bạn may mắn!", DateTime.Now.ToString("dd/MM HH:mm:ss"), _accountSession.NickName, 2000);
                                _coreAPI.SendPopup(_accountSession.AccountID, notificationSend, balance, 1);
                                _otp.PushMessageToTelegram(_accountSession.AccountID, "", notificationSend);
                            }
                            securityInfo = _otp.GetSucurityInfoWithMask((int)_accountSession.AccountID);
                            AccountDb accountDb = new AccountDb();
                            accountDb.AccountID = _accountSession.AccountID;
                            accountDb.UserName = _accountSession.AccountName;
                            accountDb.UserFullname = _accountSession.NickName;
                            accountDb.MerchantID = _accountSession.MerchantID;
                            accountDb.PlatformID = _accountSession.PlatformID;
                            accountDb.ClientIP = _accountSession.IpAddress;
                            accountDb.IsAgency = _agencyRepository.CheckIsAgency((int)_accountSession.AccountID);
                            accountDb.IsMobileActived = securityInfo.IsMobileActived;
                    ...
                    else if (model.Type == 2 || model.Type == 3) // Huy OTP va Huy số
                    {
                        ...
                        if (!_otp.VerifyOTP(_accountSession.AccountName, (int)_accountSession.AccountID, model.SecureCode, 1, securityInfo.Mobile, 0))
                            return new ResponseBuilder(ErrorCodes.ACCOUNT_OTP_NOT_MATCH, _accountSession.Language);
                        long balance = 0;
                        int res = _coreAPI.RegisterOTP((int)_accountSession.AccountID, _accountSession.AccountName, securityInfo.Mobile, (int)RegisterOTPType.UNREGISTER_OTP, out balance);
        ...
        public ActionResult<ResponseBuilder> ChangeMobileOTP([FromBody] dynamic model)
        {
            ...
            //    SecurityInfo securityInfo = _accountRepository.GetSecurityInfo(accountId);
            //    if (securityInfo != null)
            //    {
            //        if (step == 1)
            //        {
            ...
            //            var newPhoneInfo = _accountRepository.GetSecurityInfoByMobile(newMobile);
            ...
            try
            {
                ...
                if (securityInfo != null)
                {
                    ...
                    if (type == 1) // Đăng ký bảo mật đăng nhập
                    {
                        ...
                        // Xác thực qua API
                        if (_otp.VerifyOtpByNewApi(_accountSession.AccountName, _accountSession.AccountID, securityInfo.Mobile, otp, otpType) == 0)
                        {
                            if (_accountRepository.RegisterLoginOTP(accountId, accountName, 1, minBon) >= 0)
                            {
                                securityInfo = _otp.GetSucurityInfoWithMask(accountId);
                                return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, securityInfo);
                            }
                    ...
                    else if (type == 2) // Hủy đăng ký đăng nhập bằng OTP
                    {
                        ...
                        {
                            if (_accountRepository.RegisterLoginOTP(accountId, accountName, 0, minBon) >= 0)
                            {
                                securityInfo = _otp.GetSucurityInfoWithMask(accountId);
                                return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, JsonConvert.SerializeObject(securityInfo));
                            }
                    ...
                    else if (model.Type == 2) // Hủy bảo mật bằng Email
                    {
                        ...
                        if (!string.IsNullOrEmpty(securityInfo.Email))
                        {
                            ...
                            if (string.IsNullOrEmpty(model.SecureCode))
                            {
                                ...
                                //int res = _accountRepository.CreateSecurityEmail((int)accountSession.AccountID, accountSession.Mobile, securityInfo.Email, ref codeVerify);
                            ...
                            else
                            {
                                ...
                                int res = _accountRepository.VerifySecurityEmail((int)_accountSession.AccountID, _accountSession.AccountName, securityInfo.Email, Convert.ToInt32(model.SecureCode));
                                if (res > 0)
                                {
                                    //return Json(new
                                    //{
        ...
        public ActionResult<ResponseBuilder> UpdateMobile(dynamic data)
        {
            ...
            try
            {
                ...
                //    return new ResponseBuilder(ErrorCodes.NOT_SUPPORT_REGION, _accountSession.Language);
                var info = _accountRepository.GetSecurityInfoByMobile(mobile);
        ...
        public ActionResult<ResponseBuilder> GetLoyalty()
        {
            try
            {
                AccountLoyalty info = _accountRepository.GetLoyalty(_accountSession.AccountID);
        ...
        public ActionResult<ResponseBuilder> VipPointTrade(int vipPoint, int otpType, string otp)
        {
            try
            {
                ...
                if (_accountRepository.CheckUpdateMobile(_accountSession.AccountID) < 0)
                ...
                long res = _accountRepository.VipPointTrade(_accountSession.AccountID, _accountSession.AccountName, vipPoint, out vipPointCurrent);
        ...
        public ActionResult<ResponseBuilder> GetRankingVip()
        {
            try
            {
                var info = _accountRepository.GetRankingVip();
                if (info != null)
                {
        ...
        {
            return password;// Security.GetPasswordEncryp(password, username);
        }
        ...
        private bool ResetPasswordBySMS_CheckInput(ResetPassModels model, AccountEbank account, ref ReturnData returnData)
        {
            ...
            try
            {
                ...
                var status = 0;
                var listSmsBanking = _accountRepository.GetSmsbankingByAccountId(account.AccountID, ref phone, ref username, ref securitycode, ref status);
                ...
                var checkOTP = _accountRepository.CheckOtp(account.AccountID, model.Otp, 1);
        ...
        private bool ResetPasswordByEmail_CheckInput(ResetPassModels model, AccountEbank account, ref ReturnData returnData)
        {
            ...
            try
            {
                ...
                //var rnd = new Random();
                //account.ConfirmCode = rnd.Next(100000, 999999);
                //var resultUpdate = _accountRepository.UpdateUserConfirm(account.Mobile, account.ConfirmCode);
                //if (resultUpdate < 0)
                //{
        ...
        private bool SendPassWordResetToUser(AccountEbank account, string emailOrPhone, int type)
        {
            ...
            var result_reset = _accountRepository.ResetPassword(account.AccountName, account.Password, (short)type);
            ...
            //{
            //    _accountRepository.ResetPassword(account.Mobile, oldPassMd5, (short)type);
            //    return false;
            //}

            // ghi log, không thành công, update oldpass
            var error = _accountRepository.LogAccountChangepassWaiting_Insert(account.AccountID, account.AccountName, account.Password, account.Password);
            if (error < 0)
            {
                NLogManager.Info("LogAccountChangepassWaiting_Insert failed, error = " + error);
                _accountRepository.ResetPassword(account.AccountName, oldPassMd5, (short)type);
                return false;
            }
        ...
        private bool SendOTPToUser(string accountName, int accountId, string email, int type, ref string token)
        {
            try
            {
                ...
                {
                    int res = _accountRepository.CreateSecuritySMS(accountId, accountName, email, 1, ref codeVerify);
                    if (res > 0)
                    {
                ...
                else if (type == 2) // Đăng ký bảo mật bằng Email
                {
                    ...
                    {
                        int res = _accountRepository.CreateSecurityEmail(accountId, accountName, email, ref codeVerify);
                        if (res > 0)
                        {
                            token = Security.TripleDESEncrypt(OTPCHECK_SECRET_KEY, string.Format("{0}|{1}|{2}|{3}|{4}", accountId, accountName, codeVerify, tick, "ibon"));
                            StringBuilder content = new StringBuilder();
        ...
        private int DeleteOTPApp(string userName, string mobile)
        {
            try
            {
                //IOTPRepository _otpRepository = AbstractDAOFactory.Instance().IOTPRepository();
                ...
                OTPs.ResponMes responMes2 = _otpRepository.SetupAppToken(_dataOutput);
        ...
        [Authorize]
        [HttpPost("updateAccountWorldBank")]
        public ActionResult<ResponseBuilder> UpdateAccountWorldBank(dynamic data)
        {
            //string cookie = HttpContext.Current.Request.Cookies.Get(FormsAuthentication.FormsCookieName).Value;
            NLogManager.Info("UpdateAccountWorldBank");
            int bankID = data.bankID;
            long accountId = _accountSession.AccountID;
            ...
            try
            {
                ...
                long res = _accountRepository.UpdateAccountWorldBank(accountId, bankName, bankAccountName, bankAccountNumber, bankID);
                if (res >= 0)
                {
                    return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language);
                }
        ...
        [Authorize]
        [HttpGet("GetWorldBanks")]
        public ActionResult<ResponseBuilder> GetWorldBanks()
        {
            try
            {
                var locationID = _accountSession.LocationID;
                var prefix = _accountSession.PreFix;
                NLogManager.Info(string.Format("Get worldBanks param: locationID={0}, prefix={1}", locationID, prefix));
                var lst = _accountRepository.GetWorldBank(locationID, prefix);
                return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, lst);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        [Authorize]
        [HttpGet("GetAccountWorldBanks")]
        public ActionResult<ResponseBuilder> GetAccountWorldBanks()
        {
            try
            {
                var accountID = _accountSession.AccountID;
                NLogManager.Info(string.Format("GetAccountWorldBanks param: accountID={0}", accountID));
                var accountWorldBank = _accountRepository.GetAccountWorldBank(accountID);
                return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, accountWorldBank);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        public ActionResult<ResponseBuilder> GetAccountBalance()
        {
            try
            {
                var accountID = _accountSession.AccountID;
        ...
        [Authorize]
        [HttpGet("GetAccountGeneral")]
        public ActionResult<ResponseBuilder> GetAccountGeneral()
        {
            try
            {
                var accountID = _accountSession.AccountID;
                NLogManager.Info(string.Format("GetAccountGeneral param: accountID={0}", accountID));
                var data = _accountRepository.GetAccountGeneral(accountID);
        ...
        public ActionResult<ResponseBuilder> EditPassword(EditPassword param)
        {
            string lang = "en";
            try
            {
                ...
                {
                    return new ResponseBuilder(ErrorCodes.NEW_PASSWORD_NOT_MATCH, _accountSession.Language);
                }
                var accountID = _accountSession.AccountID;
                NLogManager.Info(string.Format("EditPassword param: accountID={0}", accountID));
                var response = _accountRepository.ChangePassword(accountID, param.OldPassword, param.NewPassword);
                if (response >= 0)
                    return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language);
        ...
        [Authorize]
        [HttpPost("EdiAccountGeneral")]
        public ActionResult<ResponseBuilder> EdiAccountGeneral([FromBody] dynamic data)
        {
            string lang = "en";
            try
            {
                ...
                var accountID = _accountSession.AccountID;
                var result = _accountRepository.UpdateInfo(accountID, email, mobile, "", avatar, nickname, locationID);
                NLogManager.Info(string.Format("UpdateSecurity: EdiAccountGeneral = {0}, mobile = {1}, passport = {2}, result = {3}", accountID, mobile, passport, result));
                ...
                {
                    return new ResponseBuilder(ErrorCodes.EMAIL_ESXIST, _accountSession.Language);
                }
                else if (result == -102)
                {
                    return new ResponseBuilder(ErrorCodes.ACCOUNT_MOBILE_ALREADY_USED, _accountSession.Language);
                ...
                {
                    return new ResponseBuilder(ErrorCodes.NICKNAME_ALREADY_USED, _accountSession.Language);
                }
                return new ResponseBuilder(ErrorCodes.ACCOUNT_UPDATE_FAILED, _accountSession.Language);
            }
            catch (Exception ex)
        ...
        [Authorize]
        [HttpGet("GetLoginSessions")]
        public ActionResult<ResponseBuilder> GetLoginSessions()
        {
            string lang = "en";
            try
            {
                var accountID = _accountSession.AccountID;
                var result = _accountRepository.GetLoginSessions(accountID);

                if (result.Count >= 0)
                {
                    return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, result);
                }
                return new ResponseBuilder(ErrorCodes.FAIL, _accountSession.Language);
            }
            catch (Exception ex)
            {
                NLogManager.Exception(ex);
        ...
        [Authorize]
        [HttpPost("RemoveLoginSession")]
        public ActionResult<ResponseBuilder> RemoveLoginSession([FromBody] dynamic data)
        {
            string lang = "en";
            try
            {
                ...
                var accountID = _accountSession.AccountID;
                var result = _accountRepository.RemoveLoginSession(sessionID, accountID);
