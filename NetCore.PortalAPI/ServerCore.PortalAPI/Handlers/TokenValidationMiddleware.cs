...
namespace ServerCore.PortalAPI.Handlers
{
    public class TokenValidationMiddleware
    {
        ...
        public TokenValidationMiddleware(RequestDelegate next, IOptions<AppSettings> options, IAccountDAO accountDAO)
        {
            _next = next;
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            try
            {
                ...
                var isTokenRevoked = _accountDAO.CheckLoginSession(tokenId);
                if (isTokenRevoked < 0)
                {
