...
namespace PortalAPI.Controllers
{
    ...
    public class GuildController : ControllerBase
    {
        ...
        private readonly IGuildDAO _guildDAO;
        ...
        public GuildController(AccountSession accountSession, IAgencyDAO agencyDAO, IGuildDAO guildDAO, IAuthenticateService authenticateService, IOptions<AppSettings> options)
        {
            this._accountSession = accountSession;
            this._agencyDAO = agencyDAO;
            this._guildDAO = guildDAO;
        ...
        public ActionResult<ResponseBuilder> GameAgencies()
        {
            try
            {
                ...
                List<GameAgency> listAg = _guildDAO.GetGameAgencies();
                ...
                {
                    long agencyID = _guildDAO.GetAgencyIDByMember(_accountSession.AccountID);
        ...
        public ActionResult<ResponseBuilder> UpdateRule(dynamic data)
        {
            ...
            try
            {
                ...
                int responseStatus = _guildDAO.UpdateRule(_accountSession.AccountID, content);
        ...
        public ActionResult<ResponseBuilder> GameAgencyDetail()
        {
            try
            {
                ...
                if (AdminGuild != null)
                {
                    AdminGuild.Members = _guildDAO.GetListMember(_accountSession.AccountID, 1);
                    AdminGuild.MemberRequests = _guildDAO.GetListMemberOther(_accountSession.AccountID, 0);
                ...
                else // Member
                {
                    ...
                    if (agencyID > 0)
                    {
                        ...
                        if (MemGuild != null)
                        {
                            MemGuild.Members = _guildDAO.GetListMember(MemGuild.AccountID, 1);
        ...
        public ActionResult<ResponseBuilder> RequestJoinAgency(dynamic data)
        {
            ...
            int res = _guildDAO.RequestJoinAgency(agencyID, _accountSession.AccountID, _accountSession.AccountName, _accountSession.NickName);
        ...
        public ActionResult<ResponseBuilder> GetListMember(dynamic data)
        {
            try
            {
                ...
                List<GameMember> list = _guildDAO.GetListMember(agencyAccountId, status);
        ...
        public ActionResult<ResponseBuilder> AcceptRequest(dynamic data)
        {
            try
            {
                ...
                if (agency.AgencyID < 0 || agency.LevelID > 1)
                {
                    ...
                    List<GameJoinRequest> rqJoined = _guildDAO.GetListRequestJoinAgency((int)_accountSession.AccountID, 1);
                    if (rqJoined == null || rqJoined.Count < 1) // Chưa join Bang nào
                    {
                        return new ResponseBuilder(ErrorCodes.FORBIDDEN_ACCESS, _accountSession.Language);
                    }
                    // Đã gửi yêu cầu join bang nào chưa?
                    List<GameJoinRequest> rq = _guildDAO.GetListRequestJoinAgency((int)_accountSession.AccountID, 10);
                ...
                int res = _guildDAO.AcceptRequestInfo(agencyId, nickName, status, isAgency);
                if (res == -96)
                {
