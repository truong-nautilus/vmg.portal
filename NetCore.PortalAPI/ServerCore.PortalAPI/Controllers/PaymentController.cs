...
namespace ServerCore.PortalAPI.Controllers
{
    ...
    public class PaymentController : ControllerBase
    {
        ...
        public PaymentController(AccountSession accountSession, IDataService dataService, IOptions<AppSettings> options, IPaymentDAO paymentDAO)
        {
        ...
        public async Task<ActionResult<ResponseBuilder>> DepositBankRequest(BankChargeRequest bankChargeRequest)
        {
            ...
            var _id = _paymentDAO.DepositBankRequest(_bankLogsObj);
            if (_id <= 0)
            {
            ...
            _paymentDAO.DepositBankRequestUpdate(_id, res.bank ?? "", res.account ?? "", res.bank_name ?? "", res.content ?? "", "", res.status == "1" ? 1 : -1);
        ...
        public async Task<ActionResult<ResponseBuilder>> CashoutBankRequest(WithdrawBankRequestModel request)
        {
            try
            {
                ...
                long balanceBefore = _paymentDAO.GetBalance(_accountSession.AccountID);

                if (balanceBefore < requestBalance)
                {
                    return Ok(new { code = 1, description = "Số dư tài khoản không đủ." });
                }
                var _deductResult = _paymentDAO.DeductAccount(1000071, "0c659d039705f071385f7e287470fce7", _accountSession.AccountID, _accountSession.AccountName ?? "", request.amount, 0, "Rút tiền", 0 + "", 0, 0, IPAddressHelper.GetRemoteIPAddress(Request.HttpContext, true));
                ...
                var _id = _paymentDAO.CashoutBankInsert(_cashoutLogsObj);
                if (_id <= 0)
                {
                ...
                var _logObj = _paymentDAO.GetCashoutBankById(_id);
                if (res.status == 1)
                {
                    _paymentDAO.CashoutUpdateStatus(_logObj.id, 1);
        ...
        public async Task<ActionResult<ResponseBuilder>> DepositMomoRequest(string amount, string urlCallback)
        {
            try
            {
                ...
                var _id = _paymentDAO.DepositMomoRequestInsert(_momoLogsObj);
                if (_id <= 0)
                {
                    return new ResponseBuilder(ErrorCodes.SERVER_ERROR, _accountSession.Language);
                }
                ...
                _paymentDAO.DepositMomoRequestUpdate(_id, "MOMO", res.phone, res.name, res.content, string.Empty, res.status, res.redirectLink, res.deepLink);
        ...
        public ActionResult<ResponseBuilder> MomoCallback(string request_id, string status, long amount, string message, string signature)
        {
            try
            {
                ...
                var _log = _paymentDAO.GetMomoLogById(request_id);
                ...
                _paymentDAO.UpdateCallback(_log.id, _realAmount, _virtualAmount, "", status == "1" ? 2 : -2);

                var _topupResult = _paymentDAO.TopupAccount(1000146, "218312118fc3b91578616b0d746e6be4", (int)_log.accountId, _log.accountName ?? "", (int)_virtualAmount, message, 0, 0, 0, IPAddressHelper.GetRemoteIPAddress(Request.HttpContext, true));
        ...
        public async Task<ActionResult<ResponseBuilder>> CashoutMomoRequest(string requestId, string bankAccount, string amount)
        {
            try
            {
                ...
                var _id = _paymentDAO.CashOutMomoRequestInsert(_momoLogsObj);
                if (_id <= 0)
                {
                    return new ResponseBuilder(ErrorCodes.SERVER_ERROR, _accountSession.Language);
                }
                ...
                _paymentDAO.CashOutMomoRequestUpdate(_id, res.status, Convert.ToInt64(amount));
        ...
        public ActionResult<ResponseBuilder> CashoutMomoCallback(string request_id, string status, long amount, string signature)
        {
            try
            {
                ...
                var _log = _paymentDAO.GetMomoOutLogByRequestId(request_id);
                if (_log == null || _log.Id <= 0)
                {
                    return Ok(new { code = 404, description = "Giao dịch không hợp lệ" });
                }
                ...
                //var _topupResult = _paymentDAO.TopupAccount(1000146, "218312118fc3b91578616b0d746e6be4", (int)_log.accountId, _log.accountName ?? "", (int)_virtualAmount, string.Empty, 0, 0, 0, IPAddressHelper.GetRemoteIPAddress(Request.HttpContext, true));
        ...
        public async Task<ActionResult<ResponseBuilder>> DepositCardRequest(string cardAmount, string cardSeri, string cardCode, string cardType)
        {
            try
            {
                ...
                var _id = _paymentDAO.DepositCardRequestInsert(_cardLogsObj);
                if (_id <= 0)
                {
                    return new ResponseBuilder(ErrorCodes.SERVER_ERROR, _accountSession.Language);
                }
                ...
                _paymentDAO.DepositCardRequestUpdate(_id, res.trans_code.ToString(), res.message, res.status == 0 ? 1 : -1);
        ...
        public ActionResult<ResponseBuilder> CardCallback(string status, long amount, long real_amount, string tran_id, string card_seri, string card_code, string request_id, string message, string signature)
        {
            try
            {
                ...
                var _log = _paymentDAO.GetCardLogById(request_id);
                if (_log == null || _log.id <= 0)
                {
                    return Ok(new { code = 404, description = "Giao dịch không hợp lệ" });
                }
                ...
                _paymentDAO.CardCallbackUpdate(_log.id, _realAmount, _virtualAmount, message ?? "", status == "0" ? 2 : -2);

                var _topupResult = _paymentDAO.TopupAccount(1000146, "218312118fc3b91578616b0d746e6be4", (int)_log.accountId, _log.accountName ?? "", _realAmount, message, 0, 0, 0, IPAddressHelper.GetRemoteIPAddress(Request.HttpContext, true));
        ...
        public ActionResult<ResponseBuilder> BankCallback(string request_id, string bank, int status, long amount, string message, string signature)
        {
            ...
            var _log = _paymentDAO.GetBankLogById(_logId);
            if (_log == null)
            {
                return Ok(new { statusCode = 404, message = "Giao dịch không hợp lệ" });
            }
            ...
            var _realAmount = amount;
            var _virtualAmount = _realAmount;
            _paymentDAO.BankCallbackUpdate(_log.id, _realAmount, _virtualAmount, message ?? "", status == 1 ? 2 : -2);
            if (status == 1)
            {
                var _topupResult = _paymentDAO.TopupAccount(1000146, "218312118fc3b91578616b0d746e6be4", (int)_log.accountId, _log.accountName ?? "", (int)_virtualAmount, message, 0, 0, 0, IPAddressHelper.GetRemoteIPAddress(Request.HttpContext, true));
                if (_topupResult.ResponseCode <= 0)
        ...
        public IActionResult WithdrawCallback(string request_id, int status, long amount, string signature, string message)
        {
            ...
            var _log = _paymentDAO.GetCashoutBankById(_id);
            if (_log == null)
            {
                return Ok(new { statusCode = 404, message = "Giao dịch không hợp lệ" });
            }
            ...
            _paymentDAO.CashoutBankCallbackUpdate(_log.id, amount, message, status == 1 ? 2 : -2);
            //if (request.status == 3)
            //{
