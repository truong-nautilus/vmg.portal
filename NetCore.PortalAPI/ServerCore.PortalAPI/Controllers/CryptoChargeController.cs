...
namespace ServerCore.PortalAPI.Controllers
{
    ...
    public class CryptoChargeController : ControllerBase
    {
        ...
        private readonly ICryptoChargeDAO _cryptoChargeDAO;
        private readonly IHttpContextAccessor _contextAccessor;
        private readonly WalletService _walletService;
        private readonly TatumService _tatumService;
        public CryptoChargeController(AccountSession accountSession, CacheHandler cacheHandler, ICryptoChargeDAO cryptoChargeDAO, IConfiguration configuration, WalletService walletService, TatumService tatumService)
        {
            _accountSession = accountSession;
            _cacheHandler = cacheHandler;
            _cryptoChargeDAO = cryptoChargeDAO;
        ...
        private List<CurrencyProfile> GetCurrencyProfiles()
        {
            ...
            {
                data = _cryptoChargeDAO.GetListCurrency();
        ...
        public dynamic GetListChainByCurrencyId(int currencyId)
        {
            try
            {
                ...
                if (!_cacheHandler.MemTryGet(keyHu, out data))
                {
                    data = _cryptoChargeDAO.GetListChain(currencyId);
                    _cacheHandler.MemSet(1800, keyHu, data);
                }
                return new ResponseBuilder(ErrorCodes.SUCCESS, _accountSession.Language, data);
            }
        ...
        public dynamic GetWallet(int chainId)
        {
            try
            {
                ...
                if (!_cacheHandler.MemTryGet(keyHu, out address))
                {
                    ...
                    address = _cryptoChargeDAO.GetAddress(accountId, chainId);
                ...
                var chainCreateName = _cryptoChargeDAO.GetChainCreateName(chainId);
                ...
                int resp = _cryptoChargeDAO.CreateAddress(accountId, chainId, wallet.Address, wallet.PrivateKey);
                ...
                if (chainId == 1)
                {
                    _cryptoChargeDAO.CreateAddress(accountId, 2, wallet.Address, wallet.PrivateKey);
                ...
                {
                    _cryptoChargeDAO.CreateAddress(accountId, 1, wallet.Address, wallet.PrivateKey);
        ...
        private void AddTaTum(long userId, int chainId, string chainCreateName, string address)
        {
            ...
            _cryptoChargeDAO.UpdateTaTum(userId, chainId, tatumtokenId, tatumNativeId);
        }
        ...
        public dynamic CallbackDeposit(TatumDepositReq req)
        {
            try
            {
                ...
                var resp = _cryptoChargeDAO.Deposit(req.Currency, req.Chain, req.Amount, req.CounterAddress, req.Address, req.SubscriptionType, req.BlockNumber, req.TxId, req.ContractAddress, out symbol, out currencyPrice, out rateUsdtToVnd, out amountVnd);
                string msg;

                long userId;
                _cryptoChargeDAO.GetUserIdByAddress(req.Address, out userId);
                ...
                if (userId != 0)
                {
                    var account = _cryptoChargeDAO.GetAccountInfo(userId, null, null, 1);
        ...
        public dynamic GetHistory(int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                ...
                int totalRecord;
                var list = _cryptoChargeDAO.GetHistoryByUserId(accountId, pageIndex, pageSize, out totalRecord);
                var paging = new
                {
                    List = list,
                    PageIndex = pageIndex,
        ...
        private long GetDepositRateToVnd()
        {
            ...
            string value;
            _cryptoChargeDAO.GetCoreConfig("VALUE", "CRYPTO_CHARGE_DEPOSIT_RATE_TO_VND", out value);
        ...
        private long GetWithdrawRateToVnd()
        {
            ...
            string value;
            _cryptoChargeDAO.GetCoreConfig("VALUE", "CRYPTO_CHARGE_WITHDRAW_RATE_TO_VND", out value);
            long min = long.Parse(value);
        ...
        private decimal GetMinUsdtToDeposit()
        {
            ...
            string value;
            _cryptoChargeDAO.GetCoreConfig("VALUE", "CRYPTO_CHARGE_DEPOSIT_MIN_USDT", out value);
        ...
        private decimal GetMinUsdtToWithdraw()
        {
            ...
            string value;
            _cryptoChargeDAO.GetCoreConfig("VALUE", "CRYPTO_CHARGE_WITHDRAW_MIN_USDT", out value);
            decimal num = decimal.Parse(value);
            _cacheHandler.MemSet(keyHu, num);
            return num;
        }
        ...
        public dynamic Withdraw(CurrencyWithdrawReq req)
        {
            try
            {
                ...
                var account = _cryptoChargeDAO.GetAccountInfo(accountId, null, null, 1);
                ...
                long balance;
                var resp = _cryptoChargeDAO.Withdraw(accountId, req.CurrencyId, req.ChainId, req.Amount, withdraRateToVnd, amountVnd, req.Address, status, out balance);
        ...
        private decimal GetMaxUsdtAutoToWithdraw()
        {
            ...
            string value;
            _cryptoChargeDAO.GetCoreConfig("VALUE", "CRYPTO_CHARGE_WITHDRAW_AUTO_MAX_USDT", out value);
            decimal num = decimal.Parse(value);
            _cacheHandler.MemSet(keyHu, num);
